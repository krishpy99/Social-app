...
app.post('/newscream', FBAuth, postOneScream); 
...
exports.onUserDelete = functions.region('europe-west1').firestore.document('/users/{userId}')
    .onDelete((snapshot, context) => {
        const userId = context.params.userId;
        const batch = db.batch();
        return db.collection('screams').where('userHandle','==', userId).get()
            .then(data => {
                data.forEach(doc => {
